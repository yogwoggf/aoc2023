defmodule Day2P1 do
	...
	def get_game_contribution(game_record_text) do
		[_, game_id] = Regex.run(@game_id_regex, game_record_text)
		...
		if red_cubes <= @max_red_cubes && green_cubes <= @max_green_cubes && blue_cubes <= @max_blue_cubes do
			game_id_int # Can be used as a score
		else
			0 # Can't be used as a score
		end
	end
